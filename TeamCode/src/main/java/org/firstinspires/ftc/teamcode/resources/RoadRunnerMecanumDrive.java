...
public class RoadRunnerMecanumDrive {
    // All the motors
     ...
     */
    public RoadRunnerMecanumDrive(HardwareMap hardwareMap, double ds) {
        drive = new MecanumDrive(hardwareMap, new Pose2d(0, 0, 0));
        driveSpeed = ds;
    }
    public void runDrive(Gamepad gamepad, double speed, boolean reverseSwitch, double deadzone){
        drive.setDrivePowers(new PoseVelocity2d(new Vector2d(-gamepad.left_stick_y*speed*driveSpeed, -gamepad.left_stick_x*speed*driveSpeed), -gamepad.right_stick_x*driveSpeed*speed));
    }
